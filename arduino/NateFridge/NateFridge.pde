...
#define primaryProbe 1

#define heaterOnCycles 5
#define heaterOffCycles 10

double temp[numProbes];

// fridge, liquid, freezer, arduino
// Attempting to keep freezer at 33-50
// For brewing, attempt to keep fridge between 66-69
double minTemp[numProbes] = {0,66,34};
double maxTemp[numProbes] = {80,72,65};
...
void regulateHeater() {
    ...
    heaterCounter++;
    if( heaterCounter >= heaterOnCycles && heaterIsOn==true ) {
        heaterCounter=0;
        heaterIsOn = false;
    } else if( heaterCounter >= heaterOffCycles && heaterIsOn==false ) {
        heaterCounter=0;
        heaterIsOn = true;
    }
    if( heaterCounter==0 ) {
